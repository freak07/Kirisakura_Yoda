...
if (g_ASUS_hwID == ZS660KL_EVB && g_ASUS_prjID == ZS660KL_PRJ_L)
{
	pdata->en_gpio = of_get_named_gpio(np, "qcom,nq-ven", 1);
	dev_err(dev, "sqcom,nq-ven = 1\n");
}
else
{
	pdata->en_gpio = of_get_named_gpio(np, "qcom,nq-ven", 0);
	dev_err(dev, "qcom,nq-ven = 0\n");
}
