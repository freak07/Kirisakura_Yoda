...
/*static u16 shex_to_u16(const char *hex_buf, int size)
{
    return (u16)shex_to_int(hex_buf, size);
}
*/
/*
 * Format buf:
...
static ssize_t airtrigger_station_touch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/*if (finger == 0){
		...
		finger_R = 8;
		FTS_DEBUG("airtriger L1 ,finger =%d", finger_R);
	}else{
		FTS_DEBUG("command fail \n ");
		mutex_unlock(&input_dev->mutex);
		return -EINVAL;
	}*/
	if (tmp == 0) {
...
* Return: return 0 if success
***********************************************************************/
int fts_create_sysfs(struct fts_ts_data *ts_data, struct i2c_client *client)
{
	int ret = 0;

	ts_data->station_touch_class = class_create(THIS_MODULE, "station_touch");
	if (IS_ERR(ts_data->station_touch_class)) {
		ret = PTR_ERR(ts_data->station_touch_class);
		ts_data->station_touch_class = NULL;
		goto err_create_class;
	}

	ts_data->fts_dev = device_create(ts_data->station_touch_class,
				NULL, 0, "%s", "fts");
	if (unlikely(IS_ERR(ts_data->fts_dev))) {
		ret = PTR_ERR(ts_data->fts_dev);
		ts_data->fts_dev = NULL;
		goto err_create_device;
	}

	ret = sysfs_create_link(&ts_data->fts_dev->kobj, &client->dev.kobj, "i2c");
	if (ret) {
...
err_sysfs_create_group:
	sysfs_remove_link(&client->dev.kobj, "i2c");
err_sysfs_create_link:
	device_destroy(ts_data->station_touch_class, ts_data->fts_dev->devt);
err_create_device:
	class_destroy(ts_data->station_touch_class);
err_create_class:
	FTS_ERROR("[EX]: class_create() failed!!");
	return -ENOMEM;
}
/************************************************************************
...
* Return:
***********************************************************************/
int fts_remove_sysfs(struct fts_ts_data *ts_data, struct i2c_client *client)
{
