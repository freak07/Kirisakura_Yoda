    ...
    if (index < 128) { /* single read */
        *pcb_value = 0;
        ...
        if (byte_num < BYTES_PER_TIME) {           
            ...
            w_buffer[0] = REG_SC_CB_BUF0;
            ret = fts_i2c_read_write(w_buffer, 1, pcb_value, read_num);
    for (i = 1; i < read_num; i++) {
        if (ret)
            break;
    
          if (i == read_num - 1) { /* last packet &*/
            ret = fts_i2c_read_write(NULL, 0, pcb_value + BYTES_PER_TIME * i, packet_remainder);
          } else {
            ret = fts_i2c_read_write(NULL, 0, pcb_value + BYTES_PER_TIME * i, BYTES_PER_TIME);
        }
    }
		for (i = 0; i < ((index - 128) >> 1); i++) {
            pcb_value[i] = (pcb_value[i << 1] << 8) + pcb_value[(i << 1) + 1];
...
static void save_testdata_mcap(int *data, char *test_num, int row_array_index, int col_array_index, u8 row, u8 col, u8 item_count)
{
    ...
    for (i = 0 + row_array_index; (i < row + row_array_index)  && (i < TX_NUM_MAX); i++) {
        for (j = 0 + col_array_index; (j < col + col_array_index) && (j < RX_NUM_MAX); j++) {
            if (j == (col + col_array_index - 1))
                /* the last data of the row, add "\n" */
                len = sprintf(test_data.tmp_buffer, "%d, \n",  data[i * col + j]);
            else
                len = sprintf(test_data.tmp_buffer, "%d, ", data[i * (col + col_array_index) + j]);
...
static int weak_short_get_adc_data(int all_adc_data_len, int *rev_buffer, u8 cmd)
{
    ...
    FTS_TEST_FUNC_ENTER();
    FTS_TEST_INFO("read_data_len is %d", read_data_len);
    ...
    ret = fts_i2c_read_write(w_buffer, 1, data_send, read_num);
    for (i = 1; i < packet_num; i++) {
        if (ret)
            break;
        if ((i == (packet_num - 1)) && packet_remainder) { /* last packet &*/
            ret = fts_i2c_read_write(NULL, 0, data_send + BYTES_PER_TIME * i, packet_remainder);
        } else {
            ret = fts_i2c_read_write(NULL, 0, data_send + BYTES_PER_TIME * i, BYTES_PER_TIME);
        }
    }
...
static int ft5452_noise_test(bool *test_result)
{
    int ret = 0;
    int times = 0;
    int *noise_data = NULL;
    ...
    read_reg(FACTORY_REG_DATA_SELECT, &str_value);
    write_reg(FACTORY_REG_DATA_SELECT, 0x01);
    sys_delay(20);
    ...
    ret = write_reg(REG_FIR_ENABLE, 0x01);
    if (ret) {
        FTS_TEST_SAVE_ERR( "write fir reg failed. error:%d. ", ret);
        tmp_result = false;
        goto TEST_ERR;
    }
    ...
    FTS_TEST_INFO("=========set frequecy is select mode\n" );
    ret = write_reg(REG_FREQUENCY, 0x00);
    if (ret) {
        FTS_TEST_SAVE_ERR("set frequecy select mode error. error code: %d",  ret);
        goto TEST_ERR;
    }
    sys_delay(20);

    FTS_TEST_INFO("fw_process is %d", fw_process);
    if(fw_process){
        noise_frames = ft5452_basic_thr.noise_test_frames;
        ret = write_reg(0x1C, noise_frames);
        if (ret) {
            FTS_TEST_SAVE_ERR("set frame error. error code: %d",  ret);
            goto TEST_ERR;
        }

        ret = read_reg(0x1B, &old_value);
        ret = write_reg(0x1B, 0x01);
        if (ret) {
            FTS_TEST_SAVE_ERR("write 0x1B error. error code: %d",  ret);
            goto TEST_ERR;
        }
        sys_delay(20);
        ...
        /* read rawdata, only MCAP */
        ret = read_rawdata( 1, 0xAA, va_num * 2, noise_data);
        if (ret) {
            FTS_TEST_SAVE_ERR("read rawdata fail!");
            goto TEST_ERR;
        }
        
        ret = read_reg(0x0D, &noise_value);
        if (ret) {
            FTS_TEST_SAVE_ERR("write 0x0D error. error code: %d",  ret);
            goto TEST_ERR;
        }
    
        show_data_mcap(noise_data, false, 0);
    
        /* to determine noise data if in range or not */
        noise_min = 0;
        noise_max = (ft5452_basic_thr.noise_test_max * noise_value * 4) /100;
        for (row = 0; row < tx_num; row++) {
            for (col = 0; col < rx_num; col++) {
                if (test_data.mcap_detail_thr.invalid_node[row][ col] == 0) continue; /* Invalid Node */
                value = noise_data[row * rx_num + col];;
                if (value < noise_min || value > noise_max) {
                    tmp_result = false;
                    FTS_TEST_SAVE_ERR("\ntest failure. node=(%d,  %d), get_value=%d, set_range=(%d, %d)\n", \
                                      row + 1, col + 1, value, noise_min, noise_max);
                }
            }
        }
    
        /* save test data to testdata.csv */
        save_testdata_mcap(noise_data, "noise test", 0, 0, tx_num, rx_num, 1);
        
        ret = write_reg(0x1B, old_value);
        if (ret) {
            FTS_TEST_SAVE_ERR("failed to switch_to_mapping,ret=%d", ret);
            goto TEST_ERR;
        }
     ...
     /*   ret = write_reg(REG_FRE_LIST, fre);
        ...
        ret = write_reg(FACTORY_REG_DATA_SELECT, str_value);
        if (ret) {
            FTS_TEST_SAVE_ERR("restore 0x06 error,ret= %d", ret);
            goto TEST_ERR;
        }
    }
...
static bool start_test_ft5452(void)
{
    ...
    for (item_count = 0; item_count < test_data.test_num; item_count++) {
        ...
        /* CODE_FT5452_NOISE_TEST */
        if (CODE_FT5452_NOISE_TEST == item_code) {
            ret = ft5452_noise_test(&temp_result);
            if (0 != ret || (!temp_result)) {
                test_result = false;
                test_data.test_item[item_count].testresult = RESULT_NG;
            } else
...
static void init_testitem_ft5452(char *ini)
{
    ...
    /* noise_test */
    GetPrivateProfileString("TestItem", "NOISE_TEST", "1", str, ini);
    ft5452_item.noise_test = fts_atoi(str);
...
static void init_basicthreshold_ft5452(char *ini)
{
    ...
    /* noise */
    GetPrivateProfileString("Basic_Threshold", "NoiseTest_Max", "20", str, ini);
    ft5452_basic_thr.noise_test_max = fts_atoi(str);
    GetPrivateProfileString("Basic_Threshold", "NoiseTest_Frames", "32", str, ini);
    ft5452_basic_thr.noise_test_frames = fts_atoi(str);
...
static void set_testitem_sequence_ft5452(void)
{
    ...
    /* noise_test */
    if (ft5452_item.noise_test == 1) {

        fts_set_testitem(CODE_FT5452_NOISE_TEST);
    }
    FTS_TEST_FUNC_EXIT();
}
