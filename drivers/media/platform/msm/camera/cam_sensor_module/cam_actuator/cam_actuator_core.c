...
{
	int rc = 0;
	cam_actuator_log_count=10;	//ASUS_BSP actuator log 10 count 
...
static void override_i2c_write_setting(struct camera_io_master *io_master_info, struct cam_sensor_i2c_reg_setting * setting, const char * op_string)
{
	...
	{
		if(cam_actuator_log_count==10){ //ASUS_BSP actuator log 10 count 
		...
		if(setting->reg_setting[i].reg_addr == 0x84)
		{
			...
			if(cam_actuator_log_count==10){ //ASUS_BSP actuator log 10 count 
				...
				cam_actuator_log_count=0; //ASUS_BSP actuator log 10 count 
			}
			else cam_actuator_log_count++; //ASUS_BSP actuator log 10 count 
		}
	}
}
//ASUS_BSP Zhengwei --- "porting actuator"
